...
combifilter <- function(dataset, non_zero_num = 4, threshold = 0.05, cutoff = 0.1, nthreads = NULL) {
    ...
    filterdf <- zerofilter(dataset, nthreads)  # Remove genes showing very little activity
    ...
    invisible(gc())
    filterdf <- sizefilter(filterdf, cutoff = cutoff, nthreads = nthreads)  # Filter by removing genes with the smallest range
    invisible(gc())
    return(filterdf)
}
