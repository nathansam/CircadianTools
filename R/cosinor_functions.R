...
#' @param save Logical. If TRUE, saves plots to working directory. Defaults to FALSE.
#' @param print Logical. If TRUE renders significant genes in the plot viewer. Defaults to TRUE.
#' @param path The path to be used for saving plots to. Only used if save is TRUE. If not specified then path will be based off name of the \code{results} object.
#' @return Prints or saves ggplot2 object(s).
#' @examples
...
#'
#' @export
CosinorSignificantPlot <- function(results, dataset, number = 10, period = 24, print = TRUE, save = FALSE, path = NULL) {
    results <- results[order(results$pVal), ]  # Order by most significant p-value

    if (is.null(path) == TRUE) {
        path <- deparse(substitute(results))  # If path is not given then use name of results object
    }
