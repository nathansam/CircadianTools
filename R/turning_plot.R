#' TurningPlot:
#' @description  Fits a spline to a given gene in a given dataset. Finds the turning points. Plots the turning points and spline.
...
TurningPlot <- function(genename, dataset, timelag = 0, print = TRUE, save = FALSE) {
    ...
    timevector <- CircadianTools::MakeTimevector(genematrix) - timelag  # makes timevector
    ...
    splineplot <- ggplot2::ggplot(ggplot2::aes(x = x, y = y), data = spline.df) + ggplot2::geom_line(color = "#008dd5",
        ...
        ggplot2::ggtitle(paste("Gene=", genename, ", Mean difference=", round(mean(diff(turning.points)), 2), " SD=",
            round(sd(diff(turning.points)), 2))) + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 1))

    for (i in 1:length(turning.points)) {
        splineplot <- splineplot + ggplot2::geom_vline(xintercept = turning.points[i], size = 1.2, alpha = 0.75,
            color = "#ba1200")
    }
    ...
    if (save == TRUE) {
        ggplot2::ggsave(paste("Turning_", genename, ".png"), splineplot, width = 10, height = 4.5, units = "in")
    }
