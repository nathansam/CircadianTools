...
#' @export
 HclustParamSelection <- function(dataset=NULL, distance=NULL, k=c(2,5,10), metric="euclidean", nthreads=4){
...
#' @export
ClusterParamSelection <- function(dataset=NULL, distance=NULL,k=c(2,5,10), method=c('pam', 'hclust', 'diana'),metric="euclidean",nthreads=2,save.plot=TRUE,save.df=TRUE, path=NULL ){
  if (is.null(dataset)==TRUE & is.null(distance)==TRUE){
    # Check that either a dataset or a distance matrix has been provided
    stop("Either a transcriptomics dataset or a distance matrix needs to be provided!")
  }
