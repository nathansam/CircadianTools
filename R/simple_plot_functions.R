...
BasicPlot <- function(genename, dataset, timelag = 0, method = "median", print = TRUE, save = FALSE, path = NULL) {
    ...
    graphic <- ggplot2::ggplot(data = genedata, ggplot2::aes(x = timevector, y = activity)) + ggplot2::geom_line(ggplot2::aes(x = timevector,
        y = new.average), size = 1, color = "#412d6b") + ggplot2::geom_point(size = 3, alpha = 0.5, color = "#008dd5")
...
CompPlot <- function(gene1, gene2, dataset, save = FALSE) {
    ...
    combined <- cbind(combined$timevector, combined$gene1_activity, combined$gene2_activity, new_mean_list, new_mean_list2)
    combined <- data.frame(combined)
    ...
    graphic <- graphic + ggplot2::xlab("Time (hours)") + ggplot2::ylab("Trancripts Per Million (TPM)") + ggplot2::geom_point(ggplot2::aes(y = gene2_activity,
                                                                                                                                          colour = "second gene"), size = 3, alpha = 0.5) + ggplot2::theme_bw() + ggplot2::geom_line(ggplot2::aes(y = mean2,
...
#' @param path The directory to be used for saving plots to. Uses the name of the dataset object if this argument is not specified)  Not used if save=FALSE
#' @examples
