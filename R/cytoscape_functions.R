#' CytoscapeFilter
#' @description Reduces the size of a file intended for Cytoscape by filtering out the genes/clusters which are not correlated
#' @param cytoscape.file A dataframe object generated by CytoscapeFile
#' @param threshold Positive genes below this threshold and negative genes above the additive inverse of this threshold will be removed.
#' @examples
...
CytoscapeFilter <- function(cytoscape.file, threshold = 0.9, save = TRUE, filename = NULL) {
    ...
    if (save == TRUE) {
        if (is.null(filename) == TRUE) {
            filename <- deparse(substitute(cytoscape.file))  # If filename is not given then use name of cluster.dataset object
...
CytoscapeFile <- function(cor.dataset, save = TRUE, filename = NULL, nthreads = NULL) {
    if (save == TRUE) {
        if (is.null(filename) == TRUE) {
