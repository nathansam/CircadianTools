...
basicplot <- function(genename, dataset, timelag = 0, save = FALSE, print = TRUE) {
    ...
    graphic <- ggplot2::ggplot(data = genedata, ggplot2::aes(x = timevector, 
        y = activity)) + ggplot2::geom_line(ggplot2::aes(x = timevector, y = newmean), 
        size = 1) + ggplot2::geom_point(size = 3, alpha = 0.5, color = "#39A5AE")
    ...
    if (save == TRUE) {
        ggsave(paste(genename, ".png"), graphic, width = 4, heigh = 3, units = "in", 
            dpi = 600)
    }
    if (print == TRUE) {
        return(graphic)
