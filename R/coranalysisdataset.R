...
coranalysisdataset <- function(dataset, average = "median", lag = 0, par = TRUE, nthreads = NULL, save = TRUE, filename = NULL) {
    ...
    loading.values <- CircadianTools::loading_gen(length(genenames))  # Calculate milestones for printing progress
    ...
    correlationdf <- foreach(i = 1:length(genenames), .combine = cbind) %do% {
        ...
        # Calculate correlation for the ith gene with all ( possibly lagged) genes.  First column is the name of the
        # gene compared with whilst the second column is the correlations
        if (par == TRUE) 
            {
                results <- CircadianTools::coranalysispar(genename = genenames[i], dataset = dataset, lag = lag, 
                  average = average, nthreads = nthreads)
