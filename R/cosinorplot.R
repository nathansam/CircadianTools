...
cosinorplot <- function(genename, dataset, timelag = 6, period = 24, print = TRUE, save = FALSE) {
    ...
    cosinorplotobject <- CircadianTools::ggplot.cosinor.lm(cosinormodel, endtime = 21) + ggplot2::geom_point(ggplot2::aes(y = activity, 
    ...
    if (save == TRUE) {
        ggplot2::ggsave(paste("Cosinor_", genename, ".png"), cosinorplotobject, width = 10, height = 4.5, units = "in")
    }
    if (print == TRUE) {
