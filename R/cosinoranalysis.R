    ...
    for (i in 1:genenumber) {
        ...
        if (cosinor2::cosinor.detect(cosinormodel)[4] < threshold) {
            cosinorplot <- ggplot.cosinor.lm(cosinormodel, endtime = tail(timevector, n = 1) - timelag) + ggplot2::geom_point(ggplot2::aes(y = activity, 
                x = timevector), data = geneexpression, size = 3, alpha = 0.5, color = "#39A5AE") + ggplot2::ggtitle(paste("Gene=", genename, 
                ", P-value=", round(cosinor2::cosinor.detect(cosinormodel)[4], 10))) + ggplot2::theme_bw() + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 1)) + 
